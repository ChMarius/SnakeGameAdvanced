...
void Setup()
{
    ...
    bombX=rand()%width;
    bombY=rand()%height;
    if((bombX==fruitX && bombY==fruitY) || (bombX==x && bombY==y)) // If the bomb is in the same coordinates as the snake or the fruit, its coordinates are changed
    {
        bombX=rand()%width;
        bombY=rand()%height;
    }
...
void Logic()
{
    ...
    if(x>=width || x<0 || y>=height+1 || y<0 || (bombX==x && bombY==y)) {gameOver=true; std::cout<<"Stop because border cross or bomb touched";} // If the snake crosses the border, the game stops
    for(int i=0;i<nTail;++i)
    {
        if(tailX[i]==x && tailY[i]==y ) {gameOver=true; std::cout<<"Stop because snake touched itself";} //If the snake touches itself, the game stops
    }
    
    if(x==fruitX && y==fruitY )
    // Everytime the snake eats the fruit, the tail is incremented and the fruit is randomly placed
    {
        nTail++;
        Score++;
        fruitX=rand()%width;
...
int main()
{
    //If "introToGame" returns true --> the game starts, if it returns false --> the program stops
    while(introToGame())
    {
        system("cls");
        Setup();
